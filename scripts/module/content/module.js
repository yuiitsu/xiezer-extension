    ...
    this.extFilter = function() {
        ...
        parent.find('figure noscript').each(function() {
            ...
            if (html.indexOf('<img ') !== -1) {
                $(this).parent().html(html.replace(/class="(.+?)"/g, '').replace(/style="(.+?)"/g, '').replace(/width="(.+?)"/g, '').replace(/height="(.+?)"/g, ''));
            }
        });
    };
    
    this.highlightCode = function () {
        let fikaApp = document.getElementById('xiezer');
